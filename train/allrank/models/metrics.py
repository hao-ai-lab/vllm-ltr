...
def kendalltau(y_pred, y_true, ats=None, gain_function=lambda x: torch.pow(2, x) - 1, padding_indicator=PADDED_Y_VALUE,
         filler_value=1.0):
    """
    ...
    real = y_true[0].detach().cpu().numpy()
    rela, p = scipy.stats.kendalltau(pred, real)
    print("tau: ", rela, "P: ", p)

    return torch.from_numpy(np.array([[rela]]))

def acc(y_pred, y_true, ats=None, gain_function=lambda x: torch.pow(2, x) - 1, padding_indicator=PADDED_Y_VALUE,
         filler_value=1.0):
    """
    Normalized Discounted Cumulative Gain at k.
    ...
    pred = y_pred[0].detach().cpu().numpy()
    real = y_true[0].detach().cpu().numpy()
    acc = (real == pred).sum() / len(pred)
    print("acc: ", acc, pred, real)
